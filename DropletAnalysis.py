...
def get_track_roi(minpos_px, chaxis_px, crop_margin_x, crop_size_y, filter_range=None, fpath=None, test_frame=1, bkg=None, bkgcorr_off=0, filter_d=None, minmass=None):
...
def track_postproc(track_df, px_size, fps, eta, ss_maxtomean=None, save_fname=None, plot=True, verbose=0, x_off=0, params=None, update_df=True):
    if plot:
        fig, ax = plt.subplots()
        ax2 = ax.twinx()
...
def plot_lissajous(track_df, pID_subset=None, recalc_stress=False, px_size=1, fps=1, eta=1, ss_maxtomean=None, plot_alpha=0.3):
    ...
    ax[0,0].set_xlabel(r'$x$ [px]')
    ax[0,0].set_ylabel(r'$\sigma_{ext}$ [Pa]')
    ax[1,1].set_ylabel(r'$x$ [px]')
