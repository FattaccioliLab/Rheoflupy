...
# IMAGE SETTINGS
_config_Imgs = {
        'subfolder_name' : 'Data\\',   # Where the program searches for PIV output. intended to be inside root folder
        'fname_prefix' : '',
        'fname_ext' : '.tif',
        'subtract_bkg' : 'AVG_Data.tif',
        'grayscale_range' : [-64, 64],#[-20, 20],      
        'idx_range' : [810, 2649, 1],#[216, 806, 1],#[0, -1, 1],       # Range of images to load [start_idx, end_idx, step]. Set end_idx=-1 to indicate last image
        'crop_ROI' : [0, 0, 0, -20],     # ROI TO CROP [x,y,w,h]. If w is <=0, the program will take TotWidth-w instead. Same for h
        'def_imgroot' : 'C:\\Users\\steaime\\Documents\\Research\\MyProjects\\Rheofluidics\\Data\\03_OGM\\o_W_10_300_800_sTRETTOIA5_20191127_064510_PM\\'
        }

# PIV SETTINGS
_config_PIV = {
        'subfolder_name' : None,#'PIV_empty_12x12\\', #'PIV_wDrop_12x12\\',   # Where the program searches for PIV output. intended to be inside root folder
        'fname_prefix' : 'PIV',
        'fname_ext' : '.txt',
...
_config_find = {
        ...
        'save_window' : [0, -15, 200, 200], # ROI TO SAVE [x,y,w,h]
...
_config_edge = {
        'subfolder_name' : None,#'Edges_DropFrame\\', # None to skip step
        'fname_prefix' : 'EDGE_',
        'fname_ext' : '_a.txt',
        'idx_range' : [0, -1, 1],
        'idx_from_file' : 'img', # 'img'|'drop'|'none' to associate edge file to image file:
        ...
        'field_sep' : '\t',
        'hdr_len' : 0,
        'droplet_ref_frame' : [0,16], # - None if the droplet edge is defined in the main reference frame
...
_config_output = {
        ...
        'hide_axes' : True,
        'folder' : 'E:\Data\Seattle\Rheofluidics\\AnalOut\\', # Set to None not to ouput anything
        'subfolder_suffix' : '_out\\',
        'save_fullframe' : True,   # Set to false to save only zoomed (droplet-centric) videos
        'zoom_subfolder' : None,#'Zoom\\', # None not to save zoomed videos. 
        ...
        'combined_subfolder' : None,#'Combined\\', # If not None, also save a 'combined' image, of original + zoom (if present)
        ...
        'figsize' : (9.7, 3),
        'zoom_figsize' : (5.52, 5),
        'combined_figsize' : (16,9), # if None, no figure size will be specified, constrained_layout will be used instead
        'img_cmap' : 'Greys_r',
        'img_vbounds' : [-64, 64],#[0, 256],  
        ...
        'edge_plot' : None,#'g-', # None not to plot edge
        'edge_fill' : None,#{'color':'g', 'alpha':0.2}, # None or keyword dictionary with color
        ...
        'streamopts' : dict(density=[1,0.6], linewidth=1.0, color='r', cmap='hot', arrowsize=1.0, minlength=0.04), # If None, go for a quiver. Otherwise, draw a streamplot.
...
def ReadAllEdgeFiles(froot, fnames, separator=',', header=0, readForce=False, FrameIndexPos=-2):
    listRes = []
    listForce = []
    listIdx = []
    for cur_fname in fnames:
        cur_x, cur_y, cur_Fx, cur_Fy = ReadEdgeFile(froot+cur_fname, separator=separator, header=header, readForce=readForce)
    ...
    if (readForce):
        ...
        sorted_Force = None
    return sorted_Res, sorted_Force, sorted(listIdx)

def ReadEdgeFile(fname, separator=',', header=0, readForce=False):
    cur_x = []
    cur_y = []
    cur_Fx = []
    cur_Fy = []
        ...
        for line in fin:
            if icount >= header:
                #try:
                    ...
                    if (readForce):
                        cur_Fx.append(float(words[2]))
                        cur_Fy.append(float(words[3]))
                #except:
                #    pass
            icount += 1
    if (readForce):
        return cur_x, cur_y, cur_Fx, cur_Fy
    else:
                ...
                if CheckFolderExists(froot+_config['Edge']['subfolder_name']):
                    ...
                    if (_config['Prog']['Verbose'] > 0):
                        print('\n{0} {1}*{2} edge point files found in folder {3} (index range: {4})'.format(len(Edge_fnames),\
                              _config['Edge']['fname_prefix'], _config['Edge']['fname_ext'], PIV_root, _config['Edge']['idx_range']))
                    Edge_pts, Edge_forces, Edge_idx = ReadAllEdgeFiles(Edge_root, Edge_fnames, separator=_config['Edge']['field_sep'],\
                                                                       header=_config['Edge']['hdr_len'], readForce=_config['Edge']['load_forces'])
                else:
